# 📝 Change Log: Progetto *Castello dell'anima*: indice dei log tassonomici
## Intertestualità sotto sorveglianza
### *Modello TEI-driven e AI-assisted per l'analisi di citazioni, glosse e rimandi nel Castello dell'anima*

[![TEI P5](https://img.shields.io/badge/TEI-P5-334155)](https://tei-c.org/) [![Castello dell'anima](https://img.shields.io/badge/Castello%20dell%27anima-7b2d3b)](https://github.com/luciano-longo77/castello-anima-TEI-IA)

Questo indice elenca tutti i file di log relativi
agli interventi sulla tassonomia interpretativa prudenziale.
---
- **[2026-01-20] – Tassonomia interp.md**  
  Prima mappatura delle glosse e integrazione delle operazioni prudenziali.
- **[2026-02-01] – Tassonomia interp.md**  
  Creazione del primo set di categorie semantiche per @ana.
- **[2026-02-07] – Tassonomia interp.md**  
  Revisione delle categorie e armonizzazione degli identificatori XML.
- **[2026-02-10] – Tassonomia interp.md**  
  Definizione formale delle tassonomie in `<classDecl>`.
- **[2026-02-14] – Tassonomia interp.md**  
  Introduzione dei livelli di esposizione dottrinale.
- **[2026-02-17] – Tassonomia interp.md**  
  Stabilizzazione del sistema annotativo basato su @ana.
- **[2026-02-28] – Tassonomia interp.md**  
  Normalizzazione e riallineamento tassonomie–annotazione.
- **[2026-03-30] – Tassonomia interp.md**  
  Raffinamento semantico (risk, operation, impact).
- **[2026-04-18] – Tassonomia interp.md**  
  Infrastruttura metodologica e validazione del sistema.
- **[2026-04-19] – Tassonomia interp.md**  
  Stabilizzazione della tassonomia.
- **[2026-04-22] – Tassonomia interp.md**  
  Stabilizzazione e Uniformazione.
- **[2026-05-10] – Tassonomia interp.md**  
  Uniformazione prefissi delle tassonomie di processo `fase` e `workflow`.
- **[2026-06-22] – Tassonomia interp.md**  
  Normalizzazione dei riferimenti @ana e rimozione di riferimenti orfani nel teiHeader.
- **[2026-07-15] – Tassonomia interp.md**  
  Allineamento licenza, unificazione langUsage, verifica di coerenza completa tra tassonomia-gh.xml e teiHeader.
- **[2026-07-29] – Tassonomia interp.md**  
 Verifica di coerenza sistematica: XML, schema, esempi, CI, documentazione.
